...
- Zoneless: the demo application and library tests no longer depend on `zone.js`. The app uses `provideZonelessChangeDetection()`; tests use real `async`/`await` instead of `fakeAsync`/`tick`.
